			...
			function Tab:AddSection(name, options)
				...
				function SubSection:AddDropdown(name, list, options, callback)
					...
					Dropdown.Frame = SelfModules.UI.Create("Frame", {
						...
						-- Search box (hidden until toggled)
						SelfModules.UI.Create("TextBox", {
							Name = "Search",
							BackgroundColor3 = SelfModules.UI.Color.Add(Library.Theme.SectionColor, Color3.fromRGB(5, 5, 5)),
							Position = UDim2.new(0, 5, 0, 40),
					...
					function Dropdown:GetHeight()
						-- base 42 + search box (35) + list area (max 5 * 27)
						return 42 + (Dropdown.Toggled and (35 + math.min(#Dropdown.List, 5) * 27) or 0)
					end
					...
					Dropdown.Frame.Search:GetPropertyChangedSignal("Text"):Connect(function()
						...
						for _, child in next, Dropdown.Frame.Holder.List:GetChildren() do
                    ...
                    function Dropdown:Toggle(bool)
                        ...
                        Dropdown.Frame.Search.Visible = bool
                ...
                -- MultiDropdown (searchable multi-select)

                function SubSection:AddMultiDropdown(name, list, options, callback)
                    local a, b, c = list, options, callback
                    local opts, cb, items = {}, nil, {}
                    ...
                    -- populate options
                    for _, v in next, Multi.Options do
                        ...
                        item.Activated:Connect(function()
                            ...
                            pcall(task.spawn, Multi.Callback, Multi.Value)
                        end)
                    end
                    ...
                    Multi.Frame.Holder.InputBegan:Connect(function(input)
                        if input.UserInputType == Enum.UserInputType.MouseButton1 then
                            ...
                            local targetH = Multi.Toggled and (#Multi.Options * 32 + 80) or 32
                    ...
                    self.List[#self.List + 1] = Multi
                    Multi.Frame.Parent = self.Frame.Holder.List
