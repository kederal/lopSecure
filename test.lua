	...
	function Window:AddTab(name, options)
		...
		function Tab:AddSection(name, options)
			...
			function SubSection:AddLabel(name)
				...
				Label.Frame.Parent = SubSection.Frame.List
			...
			function SubSection:AddDualLabel(options)
				...
				DualLabel.Frame.Parent = SubSection.Frame.List
			...
			function SubSection:AddClipboardLabel(name, callback)
				...
				ClipboardLabel.Frame.Parent = SubSection.Frame.List
			...
			function SubSection:AddSlider(name, min, max, default, options, callback)
				...
				Slider.Frame.Parent = SubSection.Frame.List
			...
			-- MultiDropdown (supports both signatures: (name, list, options, callback) and (name, options, callback))
			function Section:AddMultiDropdown(name, ...)
				local a, b, c = ...
				local list, opts, callback

				if type(a) == "table" and a.options ~= nil then
					-- (name, optionsTable, callback)
					opts = a
					list = a.options or {}
					callback = b
				else
				...
				-- Set Defaults (supports array or map)
				if type(MultiDropdown.Default) == "table" then
					if #MultiDropdown.Default > 0 then
						for _, v in ipairs(MultiDropdown.Default) do
							MultiDropdown.Value[v] = true
						end
					else
						for k, v in pairs(MultiDropdown.Default) do
							if v then MultiDropdown.Value[k] = true end
						end
					end
				end

				-- reflect default visuals
				for _, optName in next, MultiDropdown.Options do
					if MultiDropdown.Value[optName] then
						local btn = MultiDropdown.Frame.OptionList:FindFirstChild(optName)
						if btn and btn:FindFirstChild("Holder") and btn.Holder:FindFirstChild("Overlay") then
							btn.Holder.Overlay.BackgroundTransparency = 0.8
							btn.Holder.Overlay.BackgroundColor3 = Library.Theme.Accent
						end
					end
				end
					...
					function Tab:AddSection(name, options)
						...
						function SubSection:AddPicker(name, options, callback)
							...
							Picker.Frame = SelfModules.UI.Create("Frame", {
								...
								Picker.Frame.Parent = SubSection.Frame.List
							...
							function Section:AddSubSection(name, options)
								...
								local function toggleSubSection(bool)
									...
									tween(SubSection.Frame, 0.5, { Size = UDim2.new(1, -10, 0, SubSection:GetHeight()) })
								...
								function SubSection:AddButton(name, callback)
									...
									Button.Frame.Parent = SubSection.Frame.List
